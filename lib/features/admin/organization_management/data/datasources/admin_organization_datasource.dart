...
/// Unified datasource for ALL admin organization Firestore operations.
///
/// Merges previous FirestoreOrganizationDatasource (reads) and
/// AdminOrganizationDatasource (writes) into a single cohesive datasource
/// for the admin role.
@lazySingleton
class AdminOrganizationDatasource {
  ...
  Stream<OrganizationEntity> watchById(String orgId) {
    _logger.debug('AdminOrganizationDatasource: watchById → orgId=$orgId');
  ...
  Future<OrganizationEntity?> getByAdminUid(String adminUid) async {
    _logger.debug(
      'AdminOrganizationDatasource: getByAdminUid → adminUid=$adminUid',
    );
    try {
      ...
      if (query.docs.isEmpty) {
        _logger.debug(
          'AdminOrganizationDatasource: no org found for adminUid=$adminUid',
        );
        return null;
      }
      ...
      _logger.error(
        'AdminOrganizationDatasource: getByAdminUid failed '
        'adminUid=$adminUid',
        e,
        st,
      );
      ...
      _logger.error(
        'AdminOrganizationDatasource: getByAdminUid unexpected error '
        'adminUid=$adminUid',
        e,
        st,
      );
  ...
  /// Creates a new organization document and updates the admin user document.
  ...
  /// Updates the organization document with new profile data.
